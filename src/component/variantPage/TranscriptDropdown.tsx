...
> {
    public render() {
        const canonicalTranscriptId =
            ...
            this.props.canonicalTranscript.transcriptId;
        let otherTranscriptIds =
            this.props.otherTranscript &&
            this.props.otherTranscript.map(
                transcript => transcript.transcriptId
            ); // TODO remove the canonical id

        otherTranscriptIds = _.filter(
            otherTranscriptIds,
            id => id !== canonicalTranscriptId
        );
        let otherOptions = otherTranscriptIds.map(transcriptId => {
        ...
        const options = [
            {
                ...
                options: [
                    {
                        value: canonicalTranscriptId,
                        label: canonicalTranscriptId,
                    },
                ],
            },
            {
            ...
            <span className={classNames('small')}>
                <Select
                    ...
                    onChange={(option: { value: string; label: string }) =>
                        this.props.onTranscriptSelection(option)
                    }
